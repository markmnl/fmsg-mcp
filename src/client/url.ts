...
export function normalizeApiUrl(value: string, allowInsecureHttp = false, name = "FMSG_API_URL"): string {
  let url: URL;
  try {
    url = new URL(value);
  } catch {
  ...
  if (url.protocol === "http:" && !isLoopbackHost(url.hostname) && !allowInsecureHttp) {
    throw new Error(`${name} must use HTTPS outside loopback; explicitly enable FMSG_ALLOW_INSECURE_HTTP=1 only for a trusted development/private network`);
  }
