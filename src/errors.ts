...
export function describeError(error: unknown, address?: string): string {
  if (error instanceof FmsgHttpError) {
    const descriptions: Record<number, string> = {
      400: "fmsg host rejected the request", 401: "fmsg API key was rejected", 403: "not permitted",
    ...
    const guidance = error.status === 401 ? "The key may be revoked or expired; ask the user for a replacement."
